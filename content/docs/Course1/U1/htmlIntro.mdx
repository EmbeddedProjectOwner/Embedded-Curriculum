...
<> 
 ...
 <>HTML has very a basic <HoverCardInline Trigger={"syntax"} Content={'Definition: Syntax, a way a language is written or interpretted.'}></HoverCardInline> ``(Syntax: A way that a language is written or spoken/interpreted)`` and it works with CSS and JavaScript.</>
    ...
    In HTML you can use an opening an closing tag (``<>{:tsx}``) and a HTML tag name (``<tagname/>{:tsx}``) to represent UI.
    ```html
        <> <!-- A Opening Tag -->
        </> <!-- A Closing Tag -->
    ```
...
<DocComponents.Callout.Callout type="warn" title="Important to Note!">It is best practice, and **HIGHLY** recommended to end all elements with a closing tag.  <br/><br/> *Elements may work without a closing tag*; however, a lot of technologies/frameworks that you'll learn later on will require a closing tag. (There are a few exceptions, ex: `<!DOCTYPE html>{:html}`)</DocComponents.Callout.Callout>
...
We already defined, DOM; however there are some important terms to define in a document tree.
So lets do that before we continue. (These aren't all of them)

A **Document Tree** is essentially a hierarchy, similar to a family tree, where elements are related as parents, children, ancestors, descendants, or siblings.

### Parent
A parent in HTML is an element that is the "container" or holder of another element.

<DocComponents.Callout.Callout type="info">
    Quick definitions:

    `Nested` - Meaning underneath or inbetween an HTML element. <br/>
    *When I talk about nesting (or containing) elements*: <br/> <br/>
    `Directly` - Meaning an element is one of the first elements underneath it's parent. <br/> <br/>
    `Indirectly` - Meaning an element is not in the first quote 'group' of elements immediately underneath the parent.

    Example: (Relative to the first div )
    ```html
    <div>
        <div title="DirectlyNested">
            <div title="IndirectlyNested">
                <div title="IndirectlyNested"></div>
            </div>
        </div>
    </div>
    ```
...
<ul></ul> <!-- [!code highlight] -->
```

---

### Child
A child is an element that is directly nested inside a parent element.
...
### Descendant
A descendant is any element nested inside a parent element, at any level of the hierarchy (directly or indirectly).
...
```html
<!-- In this example, the descendants of <div> are: -->
<section></section>, <p></p> <!-- [!code highlight] -->
```

---

### Ancestor
An ancestor is any element that contains a given element, either directly or indirectly, higher up in the hierarchy.
...
```html
<!-- In this example, the ancestors of <p> are: -->
<section></section>, <div></div> <!-- [!code highlight] -->
```
...
```html
<!-- In this example, the siblings are: -->
<li>Item 1</li> and <li>Item 2</li> <!-- [!code highlight] -->
```
